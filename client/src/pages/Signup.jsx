...
const Signup = () => {
  ...
  const handleSubmit = async (e) => {
    e.preventDefault();
    try {
      const { data } = await axios.post("http://localhost:4000/api/v1/auth/signup", {
      ...
      <div className="relative z-10 max-w-md w-full p-6 bg-black/10 backdrop-blur-md rounded-xl shadow-xl border border-white/20">
        ...
        <form onSubmit={handleSubmit} className="space-y-3">
          <input
            ...
            className="w-full p-2 bg-black/40 text-white border border-gray-300 rounded-md focus:outline-none focus:ring-2 focus:ring-pink-400 text-sm sm:text-base"
            required
          />
          <input
            type="email"
